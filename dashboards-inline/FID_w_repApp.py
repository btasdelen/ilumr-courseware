...
class FIDApp(DashboardApp):
    def __init__(self, override_pars={}, override_par_files=[], show_magnitude=False, show_complex=True, enable_run_loop=False):
        super().__init__(None, Sequence(path.join('../../../dashboards-inline/FID_w_rep.py')), enable_run_loop=enable_run_loop)
        ...
        self.plot_row = pn.Row(self.plot1.figure, sizing_mode='stretch_both')
    ...
    async def run(self):
        ...
        x = np.linspace(0, self.seq.par.n_samples*self.seq.par.t_dw*self.seq.par.n_reps, self.seq.par.n_samples*self.seq.par.n_reps)
